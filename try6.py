...
def get_model_loaders():
    model_loaders = {
        'skin': {
            'CNN': lambda: build_model().load_weights('CNN_skin_classifier_weights.weights.h5'),
            'VGG16': lambda: build_model_with_saliency_Vgg().load_weights('best_VGG16_weights.weights.h5'),
            'ResNet50': lambda: build_model_with_saliency_Res().load_weights('best_ResNet50_weights.weights.h5'),
            'EfficientNetB4': lambda: build_model_with_saliency_Eff().load_weights('best_EfficientNetB4_weights.weights.h5'),
            'InceptionResNetV2': lambda: build_model_with_saliency_Inc().load_weights('best_InceptionResNetV2_weights.weights.h5')
        },
        'derm': {
            'CNN': lambda: load_model('DM_melanoma_cnn_with_saliency.keras'),
            'VGG16': lambda: load_model('DM_vgg16_model_with_saliency.keras'),
            'ResNet50': lambda: load_model('DM_best_ResNet50_model.keras'),
            'EfficientNetB4': lambda: load_model('DM_efficientnetb4_model_with_saliency.keras'),
            'InceptionResNetV2': lambda: load_model('DM_InceptionResNetV2_model.keras')
        }
    }
    return model_loaders

# Use st.cache_resource to ensure we only create the loaders once
@st.cache_resource
...
def load_specific_model(model_type, model_name):
    ...
    try:
        model = model_loaders[model_type][model_name]()
        return model
    except Exception as e:
        st.error(f"Error loading {model_type} - {model_name} model: {e}")
        return None
...
def melanoma_detection():
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            model = load_specific_model(model_type, selected_model)
            if model is None:
            ...
            if model_type == 'skin':  # Multi-class output for skin images
